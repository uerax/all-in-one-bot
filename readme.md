...
> **💡 项目重构背景与设计初衷**  
> 随着原版本功能的不断叠加，代码库逐渐过载臃肿，且大量第三方外部 API 接口因失效或结构变更导致维护成本极高。为了给用户提供更加稳定、极速、低资源占用且易于维护扩展的 Telegram 机器人体验，我们新建了 `lite` 分支进行全面架构升级：
> - ⚡ **精简与精炼**：剥离冗余沉淀代码，精选并移植原版本中最实用、高频且稳定的核心功能。
> - 🧩 **现代化架构**：引入模块化的 `Handler` 注册路由与 `Provider` 多源解耦接口，彻底告别单体复杂逻辑，具备极强的扩展性。
> - 🚀 **持续演进**：在保持轻量与高可靠性的前提下，积极准备并持续开发更多高价值的新功能。
